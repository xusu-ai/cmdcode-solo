...
- **👤 Multi-User Physical Isolation** — Each user has a dedicated folder (`/user_data/{id}/Memory/`) with Linux permissions 0700, counted under the 100MB quota.
...
```
cmdcode-solo/
├── ui.html                        # Complete frontend Agent with memory retrieval (~84KB)
├── proxy.php                      # API proxy + memory system core (~68KB)
├── config.enc.php                 # AES-256-CBC encrypted configuration
├── long-task-cron-worker.sh       # Universal async Worker (~5.8KB)
...
|------|------|-------------|
| **ui.html** | ~84 KB | Frontend chat interface, zero framework dependencies. Built-in AI Agent conversation, file manager, image viewer, audio/video playback, user authentication, and more |
| **proxy.php** | ~68 KB | Multi-provider API proxy, solves browser CORS. Supports MiniMax (triple-key rotation failover) and OpenCode Go, includes user auth, file system, memory system, remote Bash, share links, web scraping |
...
| **long-task-cron-worker.sh** | ~5.8 KB | Cron-triggered async task worker for long-running MiniMax music/video/memory extraction tasks |
...
- **Storage**: A heavy user generates ~28 MB/year (within the 100MB quota for 3+ years).
...
The core application code (HTML + CSS + JS) is ~84KB, suitable for learning vanilla web development or as a lightweight AI application base.
